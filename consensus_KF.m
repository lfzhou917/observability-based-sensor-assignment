function [x_hat_tplus1, y_hat_tplus1, Sigma_hat_tplus1, x_true, y_true] = consensus_KF (...
...
%     Z(i) =0.5 * (norm([Xr(i)-x_true; Yr(i)-y_true]))^2 + noise(ID(i)); %1/2*(x-y)^2
...
% just for robot i, we need to calculate the measurements
for i = 1 : length(Xr)  % for all the robots
    %distance-like measurements
    Z(i) =0.5 * (norm([Xr(i)-x_true; Yr(i)-y_true]))^2 + noise(ID(i)); %1/2*(x-y)^2
    
    % estimate from i's robot
    X_hat_t(i,:) = [x_hat_t(i); y_hat_t(i)];
    ...
    X_hat_tplus1_minus(i,:) = X_hat_t(i,:);
    
    z_hat(i) = 0.5 * (norm(X_hat_tplus1_minus - [Xr(i); Yr(i)]))^2;
    H(i,:) = (X_hat_tplus1_minus-[Xr(i); Yr(i)])';
    
    % residual covariance
    S(i,:) = H(i,:)*Sigma_hat_t*H(i,:)' + R;

    k(i,:) = Sigma_hat_t(2*i-1:2*i,:)*H(i,:)' * inv(S(i,:));
    
    
    
end
% we only need Z(1) for robot i
