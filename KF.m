function [x_hat_tplus1, y_hat_tplus1, Sigma_hat_tplus1] = KF (...
   tarx_true,tary_true, x_hat_t, y_hat_t, Sigma_hat_t, Xr, Yr, ID, t)
...
% yc = 3 - sin(0);
% 
% x_true = u_max*cos((t-1)/Omega) + xc;
% y_true = u_max*sin((t-1)/Omega) + yc;
...
for i = 1 : length(Xr)  % for all the robots
     %Z(i) = norm([Xr(i)-tarx_true; Yr(i)-tary_true]) + noise(ID(i));
   Z(i) =0.5 * (norm([Xr(i)-tarx_true; Yr(i)-tary_true]))^2 + noise(ID(i)); %1/2*(x-y)^2
